...
def write_email(mail_input_file, task, results_file_list):
    with open(mail_input_file, 'w') as f:
        ...
        f.write('\n@top_limit %s' % task.get('top_limit', ''))
        try:
...
def worker(task):
    ...
    if task['do_top_downloads']:
        ...
        fetch_top_downloads(lang, country, input_file, top_download_output_file, task.get('task_tag', ''),
                            task.get('top_limit', 10))
