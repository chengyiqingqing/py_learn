...
def update_new_user_report(tmp_sql_file):
    dt_str = datetime.datetime.now().strftime('%Y%m')
    update_sql = '''
        REPLACE INTO  new_user_report(`date`,`package`,`new_user_count`)
        SELECT `date`,`package`,count(`client_id`) AS `new_user_count`
            FROM
            (
                SELECT DISTINCT date_format(`arrival_time`,'%Y-%m-%d') AS `date`,`package`,`client_id`
                FROM `install_event_{dt_str}`
...
def main():
    ...
    update_new_user_report(tmp_sql_file)
    # 执行 SQL
