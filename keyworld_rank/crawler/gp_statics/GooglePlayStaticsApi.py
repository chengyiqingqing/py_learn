...
class GooglePlayStaticsApi(object):
    ...
    def try_func(self, locals_env, func, default_value):
        try:
            return eval(func, None, locals_env)
        except Exception as e:
            self.log("try_func():Exception: %s" % e)
    ...
    def fetch_users_internal(self, package_name, start_date, stop_date=None, is_inner=True, which_payload=0):
        ...
        try:
            _dict = {}
            view_list = self.try_func(locals(), 'res["result"]["1"][0]["1"]', [])
            install_list = self.try_func(locals(), 'res["result"]["1"][1]["1"]', [])
    ...
    def fetch_conversion(self, package_name, start_date, duration='day'):
        ...
        conversion_res_list = []
        try:
            conversion_list = self.try_func(locals(), 'res["result"]["1"][0]["1"]', [])
            total_conversion_list = self.try_func(locals(), 'res["result"]["1"][1]["1"]', [])
...
def get_package_list():
    ...
    name_map = {
        "Weather Widget Theme Dev Team": "luoluo@amberweather.com",
        "Amber Mobile Limited": "mars@amberweather.com",
        "Amber Widgets Dev Team": "amberweatherapp@gmail.com",
        "Clock & Weather Theme Dev": "amberwidgetthemes@gmail.com",
        "Amber Utils Dev Team": "amberutilapps@gmail.com"
    }
    name_map2 = {}
    ...
    data = {}
    for account_name in account_auth_list:
        print account_name
        auth = google_play_statics_auth_list[account_name]
        account = auth["account"]
        dev_acc = auth["dev_acc"]
        xsrf = auth["xsrf"]
        cookie = auth["cookie"]
        ...
        if g.status:
            print 'Success'
            account_name = name_map2[account_name]

            data[account_name] = pkg_list
        else:
            print 'Failed', pkg_list
    with open('all_account_package_map.json', 'w') as f:
